...
begin
  TSkuchainCmd.Current.DestinationPath := DestinationFolderEdit.Text;
end;

procedure TMainForm.ExecuteActionExecute(Sender: TObject);
begin
  TSkuchainCmd.Current.Execute;
  ShellExecute(0, 'open', PChar(TSkuchainCmd.Current.DestinationPath), nil, nil, SW_NORMAL);
end;

procedure TMainForm.ExecuteActionUpdate(Sender: TObject);
begin
  ExecuteAction.Enabled := (MainPageControl.ActivePage = ExecuteTab)
    and TSkuchainCmd.Current.CanExecute;
end;

procedure TMainForm.FormCreate(Sender: TObject);
begin
  MainPageControl.ActivePageIndex := 0;
  BasePathLabel.Caption := 'Base path: ' + TSkuchainCmd.Current.BasePath;
  TemplateFolderEdit.Text := TSkuchainCmd.Current.TemplatePath;
end;
...
begin
  TSkuchainCmd.Current.TemplatePath := TemplateFolderEdit.Text;
end;

procedure TMainForm.TestActionExecute(Sender: TObject);
begin
  TSkuchainCmd.Current.DestinationPath := '';
  TSkuchainCmd.Current.PrepareNewProject(SearchTextEdit.Text, ReplaceTextEdit.Text, MatchesEdit.Text);
  DestinationFolderEdit.Text := TSkuchainCmd.Current.DestinationPath;
end;
