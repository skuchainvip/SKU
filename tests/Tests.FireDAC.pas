...
, Skuchain.Core.MessageBodyReader, Skuchain.Core.MessageBodyWriter
...
type
  ...
  TSkuchainFireDACReaderWriterTest = class(TObject)
  private
...
{ TSkuchainFireDACWriterTest }

procedure TSkuchainFireDACReaderWriterTest.ArrayOfDataSets;
var
...
procedure TSkuchainFireDACReaderWriterTest.ArrayOfDataSetsInTValue;
var
  LDataSet: TFDMemTable;
  LDataSets: TArray<TFDMemTable>;
  LName: string;
...
procedure TSkuchainFireDACReaderWriterTest.ParamsInjection;
var
  LFD: TSkuchainFireDAC;
  LQuery: TFDQuery;
begin
  LFD := TSkuchainFireDAC.Create('MAIN_DB');
  try
    LQuery := LFD.CreateQuery('select * from EMPLOYEE where EMP_NO = :Custom_VALUE1', nil, False);
    try
      LFD.AddContextValueProvider(
        procedure (const AActivation: ISkuchainActivation;
...
procedure TSkuchainFireDACReaderWriterTest.Setup;
begin
...
procedure TSkuchainFireDACReaderWriterTest.TearDown;
begin
  FJSONMediaType.Free;
  FOutputStream.Free;
end;
...
initialization
  TDUnitX.RegisterTestFixture(TSkuchainFireDACReaderWriterTest);
