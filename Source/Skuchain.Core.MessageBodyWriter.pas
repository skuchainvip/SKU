...
*)
unit Skuchain.Core.MessageBodyWriter;
...
type
  ...
  TSkuchainMessageBodyRegistry = class
  private
  private
    FRegistry: TList<TEntryInfo>;
    FRttiContext: TRttiContext;
    class var _Instance: TSkuchainMessageBodyRegistry;
    class function GetInstance: TSkuchainMessageBodyRegistry; static;
  protected
    function GetProducesMediaTypes(const AObject: TRttiObject): TMediaTypeList; overload;
    function GetProducesMediaTypes(const AActivation: ISkuchainActivation): TMediaTypeList; overload;
  public
    constructor Create;
    ...
    procedure FindWriter(const AActivation: ISkuchainActivation;
//      const AMethod: TRttiMethod; const AAccept: string;
    ...
    class property Instance: TSkuchainMessageBodyRegistry read GetInstance;
...
{ TSkuchainMessageBodyRegistry }

class destructor TSkuchainMessageBodyRegistry.ClassDestructor;
begin
  if Assigned(_Instance) then
    FreeAndNil(_Instance);
end;

constructor TSkuchainMessageBodyRegistry.Create;
begin
  inherited Create;
...
destructor TSkuchainMessageBodyRegistry.Destroy;
begin
  FRegistry.Free;
  inherited;
end;

procedure TSkuchainMessageBodyRegistry.Enumerate(const AProc: TProc<TEntryInfo>);
var
  LEntry: TEntryInfo;
begin
...
procedure TSkuchainMessageBodyRegistry.FindWriter(const AActivation: ISkuchainActivation;
//  const AMethod: TRttiMethod; const AAccept: string;
  out AWriter: IMessageBodyWriter; out AMediaType: TMediaType);
var
...
class function TSkuchainMessageBodyRegistry.GetDefaultClassAffinityFunc<T>: TGetAffinityFunction;
begin
  Result :=
...
class function TSkuchainMessageBodyRegistry.GetInstance: TSkuchainMessageBodyRegistry;
begin
  if not Assigned(_Instance) then
    _Instance := TSkuchainMessageBodyRegistry.Create;
  Result := _Instance;
end;

function TSkuchainMessageBodyRegistry.GetProducesMediaTypes(
  const AActivation: ISkuchainActivation): TMediaTypeList;
var
...
procedure TSkuchainMessageBodyRegistry.RegisterWriter(const AWriterClass: TClass;
  ...
  RegisterWriter(
    ...
    begin
      ...
      if not Supports(LInstance, IMessageBodyWriter, Result) then
        raise ESkuchainException.Create('Interface IMessageBodyWriter not implemented: ' + AWriterClass.ClassName);
    end
  , AIsWritable
  , AGetAffinity
...
procedure TSkuchainMessageBodyRegistry.RegisterWriter(const AWriterClass,
...
procedure TSkuchainMessageBodyRegistry.RegisterWriter<T>(const AWriterClass: TClass;
  const AGetAffinity: TGetAffinityFunction);
var
...
procedure TSkuchainMessageBodyRegistry.RegisterWriter(
  const ACreateInstance: TFunc<IMessageBodyWriter>;
  const AIsWritable: TIsWritableFunction;
  const AGetAffinity: TGetAffinityFunction;
