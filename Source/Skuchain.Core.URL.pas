...
type
  ...
  public
    ...
    function MatchPath(AOtherURL: TSkuchainURL; const ACaseSensitive: Boolean = False): Boolean; overload; virtual;
...
constructor TSkuchainURL.Create(const AURL: string);
begin
  inherited Create;
...
procedure TSkuchainURL.BasePathChanged;
var
  LToken: string;
  LIndex: Integer;
...
class function TSkuchainURL.CombinePath(const APathTokens: array of string;
...
constructor TSkuchainURL.Create(AWebRequest: TWebRequest);
var
  LQuery: string;
begin
...
constructor TSkuchainURL.CreateDummy(const APaths: array of string; const ABaseURL: string);
begin
  Create(CombinePath([ABaseURL, CombinePath(APaths)]));
end;

constructor TSkuchainURL.CreateDummy(const APath: string; const ABaseURL: string);
begin
  Create(CombinePath([ABaseURL, APath]));
end;

destructor TSkuchainURL.Destroy;
begin
  FPathParams.Free;
  FSubResources.Free;
  FQueryTokens.Free;
...
class function TSkuchainURL.EnsureFirstPathDelimiter(const APath: string): string;
begin
  Result := EnsurePrefix(APath, URL_PATH_SEPARATOR);
end;

class function TSkuchainURL.EnsureLastPathDelimiter(const APath: string): string;
begin
  Result := EnsureSuffix(APath, URL_PATH_SEPARATOR);
end;

function TSkuchainURL.GetHasPathParams: Boolean;
begin
  Result := FPathParams.Count > 0;
end;

function TSkuchainURL.GetHasSubResources: Boolean;
begin
  Result := FSubResources.Count > 0;
end;

function TSkuchainURL.GetPathParamIndex(const AParam: string): Integer;
var
...
function TSkuchainURL.HasPathTokens(const AtLeast: Integer): Boolean;
begin
  Result := Length(FPathTokens) >= AtLeast ;
end;

function TSkuchainURL.MatchPath(APath: string; const ACaseSensitive: Boolean): Boolean;
begin
...
function TSkuchainURL.MatchPath(AOtherURL: TSkuchainURL; const ACaseSensitive: Boolean): Boolean;
var
  LIndex: Integer;
...
function TSkuchainURL.ParsePathTokens(const APath: string): TArray<string>;
var
  LPath: string;
  LIndex: Integer;
begin
...
procedure TSkuchainURL.ParseQueryTokens;
var
  LQuery: string;
...
function TSkuchainURL.QueryTokenByName(const AName: string;
...
begin
  ...
  if (not LFound) and AExceptionIfNotFound then
    raise ESkuchainException.CreateFmt('QueryParam not found: %s', [AName]);
end;

procedure TSkuchainURL.SetBasePath(const Value: string);
begin
...
procedure TSkuchainURL.SetURL(const Value: string);
begin
  if FURL <> Value then
  begin
    FURL := Value;
...
function TSkuchainURL.SubResourcesToArray: TArray<string>;
var
...
function TSkuchainURL.ToJSON: string;
var
  LObj: TJSONObject;
begin
  LObj := ToJSONObject;
...
function TSkuchainURL.ToJSONObject: TJSONObject;
begin
...
function TSkuchainURL.ToString: string;
begin
  Result := Format(
      'URL: %s' + sLineBreak
...
procedure TSkuchainURL.URLChanged;
begin
  Parse;
end;

class function TSkuchainURL.URLDecode(const AString: string): string;
begin
//  Result := TNetEncoding.URL.Decode(AString);
  Result := TIdURI.URLDecode(AString);
end;

class function TSkuchainURL.URLDecode(const AStrings: TArray<string>): TArray<string>;
var
  LIndex: Integer;
begin
...
class function TSkuchainURL.URLEncode(const AString: string): string;
begin
//  Result := TNetEncoding.URL.Encode(AString);
  Result := TIdURI.PathEncode(AString);
end;

class function TSkuchainURL.URLEncode(const AStrings: TArray<string>): TArray<string>;
var
  LIndex: Integer;
begin
  Result := AStrings; // copy on write
