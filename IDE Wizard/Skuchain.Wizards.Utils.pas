...
/// </summary>
  TSkuchainSourceFile = class(TInterfacedObject, IOTAFile)
  strict private
...
{$REGION 'TSkuchainSourceFile'}

constructor TSkuchainSourceFile.Create(const AResourceName: string);
begin
  inherited Create;
  FResourceName := AResourceName;
end;

function TSkuchainSourceFile.GetAge: TDateTime;
begin
  Result := -1;
end;

function TSkuchainSourceFile.GetSource: string;
var
  Res: TResourceStream;
  S: TStrings;
begin
